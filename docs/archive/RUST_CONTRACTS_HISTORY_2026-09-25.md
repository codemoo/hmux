...
- First-party installation lives in the Rust CLI. Thin shell/bootstrap scripts and
  test harnesses may remain, but new binary installations require no Python or compiler.
  An optional Python wrapper is compatibility-only and must select an explicit or host
  bundle rather than acting as a second installer.
- Active Go source, modules and the vendored collector are retired. Git history and
  immutable earlier release artifacts retain reference evidence; they are never current
...
The root Cargo workspace is the active native runtime. The following layout records
...
- Preserve opt-in launchd/systemd user services, explicit PATH, singleton locks,
  ...
  macOS uses an immediate owner/birth/argv/environment recheck followed by
  positive-PID TERM. Those calls are not atomic; PID reuse in that narrow interval
  remains a limitation and must not be described as eliminated.
...
- Home `home-connector.lock` is a lifetime `flock`; recovery/workspace/session/workflow
  ...
  inode or substitute an unrelated lock API. The historical mixed-version tests
  covered Go/Rust helper handoff; current tests cover Rust helper/hook concurrency
  and recovery requests.
- The historical Go gateway had process-local store mutexes rather than a gateway-wide
  cross-process state lock. Current-state rollback still stages against separate
  synthetic state and activates one verified gateway owner at a time.
- The native installer has a durable two-binary transaction and recovery journal.
  Test interrupted installation and preserve old executables/service definitions as
  rollback artifacts; hooks continue to find `hmux-agent`.
- A release replacement records version/hash, service PID/birth identity and original
...
- Go source retirement is authorized and complete in the active tree. Retain known-good
...
weaker fallback. Historical mixed-version evidence remains archived. Document one
connection generation per peer, frame direction, operation allowlists, IDs, exact
ACK units and all byte/count/time limits.
...
The shipped native runtime is Rust with efficient Protobuf Home transport. Schema and
codecs may advance only with interoperability, security, load and rollback gates.
...
managers. Package and validate each supported target before advertising it.
