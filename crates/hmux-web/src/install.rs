...
const USAGE: &str = "usage: hmux-web install-home [--lang en|ko] [--guided] [--connection-file FILE] [--source-dir DIR] [--bin-dir DIR] [--config-dir DIR] [--workspace-dir DIR] [--binaries-only | --enable-service [--url wss://host/connect --token-file FILE]]";
...
pub async fn run(args: &[OsString], stop: CancellationToken) -> io::Result<()> {
    if args == ["--help"] || args == ["-h"] {
        println!("{USAGE}");
        if locale::korean() {
            println!("\n네이티브 Home 연결 프로세스와 도우미를 설치합니다.\n\n  --connection-file 비공개 Gateway 연결 JSON을 --guided 또는 --enable-service와 함께 가져옵니다\n  --guided          작업 공간과 연결 설정을 안내합니다\n  --enable-service  자동 시작을 등록합니다. --url과 --token-file이 없으면 실행 중인 연결을 사용합니다\n  --binaries-only   설정을 변경하지 않고 실행 파일을 업데이트합니다\n\n예시:\n  hmux-web install-home --guided\n  hmux-web install-home --enable-service --url wss://hmux.example/connect --token-file ~/.config/hmux/web/connector.token\n\n자동 시작은 선택 사항입니다. 기존 작업 공간 경로는 유지됩니다.");
        } else {
    ...
    if let Some(ref file) = pairing {
        ...
        println!(
            "{}",
            locale::tr("  Private Gateway connection file validated. Token contents stay hidden.")
        );
    }
    if !options.binaries_only {
        ...
        if existing && options.workspace.is_none() {
            println!(
                "{}",
                locale::tr("  Existing configuration found. Keeping your workspace paths.")
            );
        }
    ...
    if options.binaries_only {
        ...
        tokio::task::spawn_blocking(move || hmux_install::install(&source, &target))
            ...
            .map_err(io::Error::other)??;
        active(&stop)?;
        println!("{}", locale::tr("Installed hmux-web and hmux-agent"));
    ...
    println!(
        "{}",
        locale::tr(
            "Home configured; existing paths are preserved unless --workspace-dir is supplied."
        )
    );
    let enable_service = options.enable_service;
    if enable_service {
    ...
    if !enable_service && pairing_path.is_some() {
        println!("{}", locale::tr("Keep your private connection file. Pass --connection-file to a later guided installation to connect without retyping its settings."));
    }
    Ok(())
}
#[cfg(test)]
