...
// TerminalViewPTY owns only its disposable grouped view and attached client. Home's
// original session and provider processes survive Close and context cancellation.
type TerminalViewPTY struct {
	*os.File
	Done   <-chan struct{}
...
func (p *TerminalViewPTY) Close() error {
	var err error
	p.once.Do(func() { err = p.File.Close(); p.cancel() })
	return err
}
func (p *TerminalViewPTY) Resize(cols, rows uint16) error {
...
func (p *TerminalViewPTY) Refresh(ctx context.Context) error {
...
func (p *TerminalViewPTY) refresh(ctx context.Context, foreground func(context.Context, int, string) (int, error), signalGroup func(int) error) error {
	select {
	case <-p.Done:
...
func (p *TerminalViewPTY) refreshTarget(ctx context.Context) ([]string, error) {
...
func OpenTerminalViewPTY(ctx context.Context, cfg config.HomeConfig, identity model.SessionIdentity, cols, rows uint16) (*TerminalViewPTY, error) {
	...
	name, err := newTerminalViewName()
	if err != nil {
		return nil, err
	}
	setup, cancel := context.WithTimeout(ctx, 15*time.Second)
	err = createExpectedTerminalView(setup, runner, identity.ID, identity.CreatedAt, name)
	cancel()
	if err != nil {
		return nil, err
	}
	...
	hookCancel()
	if err != nil {
		cleanupTerminalView(runner, name)
		return nil, err
	}
	args, err := catalog.TerminalViewAttachArgs(name, true)
	if err != nil {
		cleanupTerminalView(runner, name)
		return nil, err
	}
	...
	view := &TerminalViewPTY{File: terminal, Done: done, cancel: stop, runner: runner, name: name, pid: command.Process.Pid}
	go func() { _ = command.Wait(); _ = view.Close(); cleanupTerminalView(runner, name); close(done) }()
	return view, nil
}
