...
func TestWebTerminalViewWithIsolatedTmux(t *testing.T) {
...
`), 0600); err != nil {
	...
	defer cancel()
	view, err := OpenTerminalViewPTY(ctx, config.DefaultHomeConfig(), id, 80, 24)
	if err != nil {
		t.Fatal(err)
	}
	...
	stale := id
	stale.CreatedAt++
	if p, err := OpenTerminalViewPTY(ctx, config.DefaultHomeConfig(), stale, 80, 24); err == nil {
		p.Close()
...
func TestTerminalViewRefreshTargetsOnlyOwnedClient(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p := &TerminalViewPTY{Done: make(chan struct{}), runner: r, name: "hmux-e2e-view-test", pid: 42}
			signalled := false
...
func TestTerminalViewRefreshRejectsChangedPane(t *testing.T) {
