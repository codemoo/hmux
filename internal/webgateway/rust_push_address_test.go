...
func TestRustPushAddressOracle(t *testing.T) {
	type row struct {
		Address string `json:"address"`
		Allowed bool   `json:"allowed"`
	}
	cases := map[string]bool{}
	add := func(raw string) { cases[raw] = publicPushAddress(net.ParseIP(raw)) }
	...
	raw = append(raw, '\n')
	path := filepath.Join("..", "..", "tests", "fixtures", "push-v1", "go-addresses.json")
	if os.Getenv("UPDATE_HMUX_RUST_PUSH_ADDRESS_FIXTURE") == "1" {
	...
	if !bytes.Equal(raw, expected) {
		t.Fatal("Go push address policy changed")
	}
}
