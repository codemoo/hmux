#!/bin/sh
# Candidate native release bundles; does not modify Go bundles or services.
set -eu
...
for hmux_target in $hmux_targets; do
	...
	*)
	...
	mkdir -p dist
	hmux_final="dist/rust-web-$hmux_target"
	hmux_bundle=$(mktemp -d "dist/.rust-web-$hmux_target.XXXXXX")
	...
	cp deploy/web/install-home-native.py "$hmux_bundle/install-home.py"
	...
	COPYFILE_DISABLE=1 tar -czf "dist/hmux-rust-web-$hmux_target.tar.gz" -C "$hmux_bundle" .
	rm -rf "$hmux_final"
